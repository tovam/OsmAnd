...
java -Djava.io.tmpdir="$test_output" -cp "$test_libs/ktfmt.jar" org.jetbrains.kotlin.cli.jvm.K2JVMCompiler \
  ...
  "$repo_root/OsmAnd/test/java/net/osmand/test/junit/FlightSplitViewportTest.kt" \
...
java -Djava.io.tmpdir="$test_output" -cp "$classpath" org.junit.runner.JUnitCore \
  ...
  net.osmand.test.junit.FlightCalibrationTerrainTest \
  ...
  net.osmand.test.junit.FlightSplitViewportTest
cd "$repo_root"
